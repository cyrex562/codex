<template>
  <v-expansion-panels v-if="isEntityFile" class="entity-relations-panel">
    <v-expansion-panel>
...
const isEntityFile = computed(() => !!entity.value || loading.value || !!error.value);
...
function relTitle(rel: EntityRelation): string {
    const p = rel.target_path ?? rel.relation_type ?? 'Unknown relation';
    return p.split('/').pop()?.replace(/\.md$/, '') ?? p;
}

function openRelated(rel: EntityRelation) {
    if (!rel.target_path) return;
