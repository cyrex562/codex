...
#[derive(Clone)]
pub(crate) struct SharedWsStream(pub(crate) Arc<AsyncMutex<codex_client::WsStream>>);
...
impl Default for FeatureFlags {
    fn default() -> Self {
        ...
        // (e.g. CODEX_DISABLE_ML=1 turns off ml_features).
        Self {
            ml_features: std::env::var("CODEX_DISABLE_ML").is_err(),
            media_preview: std::env::var("CODEX_DISABLE_MEDIA").is_err(),
            event_sync: std::env::var("CODEX_DISABLE_SYNC").is_err(),
            diagnostics_panel: std::env::var("CODEX_DIAGNOSTICS").is_ok(),
        }
    }
}
...
fn file_cache_dir() -> Result<PathBuf, String> {
    ...
    Ok(base_dir.join("codex").join("file-cache"))
}
...
fn session_state_file_path() -> Result<PathBuf, String> {
    ...
    Ok(base_dir.join("codex").join("desktop-session.json"))
}
