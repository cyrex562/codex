...
use codex_client::FinishUploadSessionRequest;
use codex_client::ObsidianClient;
use codex_types::{
...
fn main() -> iced::Result {
    // Initialise structured logging.  Set RUST_LOG=codex_desktop=debug for verbose output.
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("codex_desktop=info")),
        )
        .with_target(false)
        .init();
...
pub(crate) enum Message {
    ...
    OutlineLoaded(Result<codex_types::NoteOutlineResponse, String>),
    OutlineSectionSelected(usize, String),
    SuggestionsRefreshPressed,
    SuggestionsLoaded(Result<codex_types::OrganizationSuggestionsResponse, String>),
    SuggestionActionPressed(usize, bool),
    SuggestionApplied(Result<codex_types::ApplyOrganizationSuggestionResponse, String>),
    UndoLastMlPressed,
    UndoLastMlLoaded(Result<codex_types::UndoMlActionResponse, String>),
    BacklinksRefreshPressed,
    BacklinksLoaded(Result<Vec<String>, String>),
    LoadTagsPressed,
    TagsLoaded(Result<Vec<codex_client::TagEntry>, String>),
    ...
    CatchUpLoaded(Result<Vec<codex_types::FileChangeEvent>, String>),
    ...
    AdminPanelToggled,
    AdminRefreshUsers,
    AdminUsersLoaded(Result<Vec<codex_types::AdminUser>, String>),
...
pub(crate) fn update(state: &mut DesktopApp, message: Message) -> Task<Message> {
    match message {
        ...
        Message::ReplayOfflineEdits => {
            ...
            Task::perform(
                async move {
                    let request = codex_types::UpdateFileRequest {
                        content,
                        last_modified: None,
                        frontmatter: None,
                    };
        ...
        Message::AdminCreateUserPressed => {
            ...
            Task::perform(
                async move {
                    client
                        .admin_create_user(&codex_types::CreateUserRequest {
...
fn run_apply_suggestion(
    ...
    suggestion: codex_types::OrganizationSuggestion,
    dry_run: bool,
) -> Task<Message> {
    let (Some(client), Some(vault_id)) = (state.client.clone(), state.selected_vault_id.clone())
    else {
    ...
    Task::perform(
        async move {
            client
                .apply_suggestion(
                    &vault_id,
                    &codex_types::ApplyOrganizationSuggestionRequest {
                        file_path,
                        suggestion,
                        dry_run,
                    },
...
mod tests {
    use super::*;
    use codex_types::EditorMode as PreferenceEditorMode;
    ...
    fn vault_selected_clears_per_vault_state() {
        ...
        s.search_results.push(codex_types::SearchResult {
            path: "a.md".into(),
            title: "a".into(),
            matches: vec![],
            score: 1.0,
