...
- `crates/librarium-mobile`: Route C thin-client command layer (vault list/get from a local JSON registry, file/directory ops over `librarium-core::FileService`, Markdown render, wiki-link/backlinks/outgoing-links, tags, frontmatter read/write, on-device Tantivy search); no frontend or Tauri-app wiring yet
